...
define GCC_TARGET_INSTALL_LIBPTHREAD
    mkdir -p $(TARGET_DIR)/usr/lib

    # Copy the real shared object to /usr/lib if not already there
    if [ -f $(TARGET_DIR)/lib/libpthread.so.0 ]; then \
        ln -sf /lib/libpthread.so.0 $(TARGET_DIR)/usr/lib/libpthread.so.0; \
    fi

    # Create the development symlink if not already exists
    if [ ! -e $(TARGET_DIR)/usr/lib/libpthread.so ]; then \
        ln -sf /usr/lib/libpthread.so.0 $(TARGET_DIR)/usr/lib/libpthread.so; \
    fi
endef

GCC_TARGET_POST_INSTALL_TARGET_HOOKS += GCC_TARGET_INSTALL_LIBPTHREAD
